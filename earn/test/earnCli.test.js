...
jest.mock('../src/main/peakEngine', () => {
  ...
  class PeakEngine extends EventEmitter {
    constructor(opts) {
      ...
      this.start = jest.fn((settings) => {
        this.settings = settings;
        if (PeakEngine.startError) throw PeakEngine.startError;
        return PeakEngine.startReturns;
      });
      this.stop = jest.fn();
      this.isRunning = jest.fn(() => true);
  ...
  PeakEngine.startError = null;
  PeakEngine.startReturns = undefined;
...
function applyDefaults(m) {
  ...
  // No PeakMiner installed unless a test says otherwise. The CLI probes for the
  // binary with fs.accessSync, and jest's fs automock resolves every path, which
  // would silently select PeakMiner in every test that only means to exercise
  // the built-in core.
  ...
  m.PeakEngine.startReturns = undefined;
}
...
function load() {
  ...
  m.PeakEngine = require('../src/main/peakEngine').PeakEngine;
...
describe('mining', () => {
  ...
  // PeakMiner is proprietary and never bundled, so 'auto' means "use it if the
  ...
  describe('engine selection', () => {
    ...
    test('uses PeakMiner when a binary is on PATH, and says so', async () => {
      const m = load();
      onPath(m);
      const p = m.run(['-a', ADDR, '--mode', 'mining', '--no-update']);
      await settle();
      expect(m.PeakEngine.instances).toHaveLength(1);
      expect(m.PearlEngine.instances).toHaveLength(0);
      expect(allOut()).toContain('/usr/bin/peakminer');
      m.PeakEngine.instances[0].emit('stopped', 0);
      await expect(p).resolves.toBe(0);
    });

    test('--miner native ignores an installed PeakMiner', async () => {
      const m = load();
      onPath(m);
      const p = m.run(['-a', ADDR, '--mode', 'mining', '--no-update', '--miner', 'native']);
      await settle();
      expect(m.PearlEngine.instances).toHaveLength(1);
      expect(m.PeakEngine.instances).toHaveLength(0);
      m.PearlEngine.instances[0].emit('stopped', 0);
      await expect(p).resolves.toBe(0);
    });
    ...
    test('--miner peak with nothing installed exits 1 in mining mode', async () => {
      const m = load();
      await expect(m.run(['-a', ADDR, '--mode', 'mining', '--no-update', '--miner', 'peak']))
        .resolves.toBe(1);
