...
jest.mock('../src/main/srbEngine', () => {
  class SrbEngine {
    constructor(opts) { this.opts = opts; SrbEngine.instances.push(this); }
  }
  SrbEngine.instances = [];
  return { SrbEngine, httpStatsFetcher: jest.fn(() => 'FETCHER') };
});

const { PearlEngine } = require('../src/main/pearlEngine');
const { SrbEngine, httpStatsFetcher } = require('../src/main/srbEngine');
...
beforeEach(() => {
  ...
  SrbEngine.instances.length = 0;
  httpStatsFetcher.mockClear();
});
...
describe('SRBMiner-Multi selected', () => {
  test('builds a SrbEngine on the resolved binary', () => {
    const r = build({
      env: { PATH: '/usr/bin' }, exists: (p) => p === '/usr/bin/SRBMiner-MULTI',
    });
    expect(r.miner).toBeInstanceOf(SrbEngine);
    expect(r.miner.opts.binPath).toBe('/usr/bin/SRBMiner-MULTI');
    expect(r.miner.opts.spawn).toBe(spawn);
    expect(r.miner.opts.fetchStats).toBe('FETCHER');
  ...
  test('says why it chose SRBMiner-Multi', () => {
    const r = build({ env: { PATH: '/usr/bin' }, exists: () => true });
    expect(r.notes[0]).toMatch(/SRBMiner-Multi/);
  });

  test('SRBMiner-Multi wins over an available native core', () => {
    const r = build({ createCore: CORE, exists: () => true, env: { PATH: '/usr/bin' } });
    expect(r.miner).toBeInstanceOf(SrbEngine);
  });
});

describe('SRBMiner-Multi requested but absent', () => {
  test('is a refusal with no engine, whatever the core situation', () => {
    for (const createCore of [CORE, null]) {
      SrbEngine.instances.length = 0;
      PearlEngine.instances.length = 0;
      const r = build({ settings: { miner: 'srb' }, createCore, exists: () => false });
      expect(r.miner).toBe(null);
      expect(r.coreMissing).toBe(false);
      expect(r.notes[0]).toMatch(/no binary was found/);
      expect(PearlEngine.instances).toHaveLength(0);
      expect(SrbEngine.instances).toHaveLength(0);
    }
  });
});
...
describe('native selected', () => {
  ...
  test('--miner native ignores an installed SRBMiner-Multi', () => {
    const r = build({
...
describe('defaults', () => {
  ...
  // forgets to pass one gets the built-in core, not a phantom SRBMiner-Multi.
