...
describe('--miner / --miner-bin', () => {
  ...
  test('defaults to auto, which prefers PeakMiner but does not require it', () => {
  ...
  test('carries an explicit engine choice through', () => {
    expect(buildSettings({ '--miner': 'peak' }, errs(), true, false).miner).toBe('peak');
  ...
  test('rejects an unknown engine and lists the choices', () => {
    const errors = [];
    buildSettings({ '--miner': 'peakminer' }, errors, true, false);
    expect(errors.join(' ')).toContain('unknown miner: peakminer');
    expect(errors.join(' ')).toContain('auto, peak, native');
  });
