...
const USAGE = [
  ...
  '                           "auto" uses PeakMiner when a binary is found and',
  '                           falls back to the built-in core. PeakMiner is faster',
  '                           (~144 vs ~111 TH/s on a 5090) but is proprietary and',
  '                           takes a 2% dev fee; it is not bundled — install it',
  '                           yourself. "native" always uses our zero-fee core.',
  '      --miner-bin <path>   Path to the PeakMiner binary (default: $PEAK_MINER_BIN,',
  '                           then `peakminer` on PATH).',
