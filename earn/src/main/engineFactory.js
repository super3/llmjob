...
const { PeakEngine, httpSummaryFetcher } = require('./peakEngine');
...
// this they would each grow the same three-way branch -- PeakMiner, our core, or
...
function createMinerEngine({
  ...
  if (sel.engine === 'peak') {
    return {
      miner: new PeakEngine({
        binPath: sel.bin,
        spawn,
        fetchSummary: httpSummaryFetcher(http),
      }),
      notes,
      coreMissing: false,
    };
  ...
  // --miner peak on a rig with no binary. Nothing to fall back to: asking for a
