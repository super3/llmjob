...
extern "C" __global__ void pearl_gemm_fold(
  ...
  // One warp per region, one lane per tile cell, operands staged in shared
  // memory with a PADDED stride.
  //
  // Reading straight from global looks cheap because the A side broadcasts —
  // lanes 0..7 share a row — but the B side is strided by k, so the eight
  // distinct columns each cost their own 32-byte transaction to deliver four
  // useful bytes. Staging fixes that; staging with the natural stride of `rank`
  // does not, because rank is a multiple of 32 and every slice then lands in the
  // same shared-memory bank: 4-way conflicts on A and 8-way on B. That is why
  // the first attempt at staging measured SLOWER than not staging at all.
  //
  // Padding the stride by one word makes consecutive slices differ by 129 words,
  // 129 % 32 == 1, so a warp reading one element from each slice touches 32
  // distinct banks and the conflicts disappear.
  extern __shared__ int32_t smem[];
  const uint32_t lane = threadIdx.x & 31u;
  const uint32_t warp = threadIdx.x >> 5;
  const uint32_t warps_per_block = blockDim.x >> 5;
  const uint32_t stride = rank + 1u;                       // the padding
  const uint32_t slice = (rows_count + cols_count) * stride;
  int32_t *sA = smem + (size_t)warp * slice;
  int32_t *sB = sA + (size_t)rows_count * stride;
...
#pragma unroll
  ...
  for (uint32_t chunk = 0; chunk < chunks; chunk++) {
    ...
    // Cooperative, coalesced: consecutive lanes take consecutive elements of the
    // same slice.
    for (uint32_t idx = lane; idx < rows_count * rank; idx += 32u) {
      ...
      sA[rr * stride + t] = (t < lim) ? Aprime[(size_t)r * k + k0 + t] : 0;
    }
    for (uint32_t idx = lane; idx < cols_count * rank; idx += 32u) {
      ...
      sB[cc * stride + t] = (t < lim) ? Bprime[(size_t)c * k + k0 + t] : 0;
    }
    __syncwarp();

    int32_t acc = 0;
    if (active) {
      const int32_t *arow = sA + (size_t)ri * stride;
      const int32_t *bcol = sB + (size_t)ci * stride;
      int32_t s0 = 0, s1 = 0, s2 = 0, s3 = 0;
#pragma unroll 8
      for (uint32_t t = 0; t < rank; t += 4) {
        s0 += arow[t] * bcol[t];
        s1 += arow[t + 1] * bcol[t + 1];
        s2 += arow[t + 2] * bcol[t + 2];
        s3 += arow[t + 3] * bcol[t + 3];
      }
      acc = (s0 + s1) + (s2 + s3);
    }
