...
$out = Join-Path $env:TEMP "pearl_kernel_stats.cubin"
& $nvcc -arch=sm_89 -std=c++17 -O3 -Xptxas=-v -cubin `
    (Join-Path $here "src\pearl_kernel.cu") -o $out -I (Join-Path $here "src") 2>&1 |
    Select-String -Pattern 'Function properties|registers|spill|smem|Compiling entry' |
